  ...
  try {
    ...
    const avgProgress =
      enrollmentCount === 0
        ? 0
        : Math.round(
            enrollments.reduce((sum, e) => sum + e.progress, 0) / enrollmentCount
          );
...
// NEW: Public — top 10 students in a course
// Ranked by lesson completions + quiz points
// IMPORTANT: must stay above GET /:id
// =========================================
router.get("/:id/leaderboard", async (req, res) => {
  try {
    ...
    // Get lesson completion counts per user for this course
    ...
    // Get quiz points earned in this course per user
    ...
    const rows = enrollments.map((e) => ({
      ...
      // Score = progress (weight 60%) + quizPoints (weight 40%, capped at 100)
      score:
        Math.round(e.progress * 0.6) +
        Math.min(Math.round((quizPointsMap[e.userId] || 0) * 0.4), 40),
    }));

    // Sort by score desc, then by lessonsCompleted desc
    rows.sort((a, b) =>
      b.score !== a.score
        ? b.score - a.score
...
// Instructor only: update course details
// =========================================
...
// Instructor or Admin: delete course
// =========================================
...
// Student only: manually update progress
// =========================================
router.patch("/:id/progress", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const updated = await prisma.enrollment.update({
      where: {
        userId_courseId: { userId: req.user.sub, courseId },
      },
      data: { progress },
    });
